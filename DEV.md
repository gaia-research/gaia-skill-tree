...
| `fuse <generic_id> …` | ✓ | — | Upserts a generic fusion node + optional suite manifest (`registry/nodes\|named\|suites`) + timeline. The prerequisite/fusion timeline event appended when prereqs are set uses action `fuse`; legacy `note` events whose details contain `via 'gaia dev fuse'` are repaired to `fuse` on rerun. No git. Rebuilds unless `--no-build`. |
...
* **Fix:** The version strings in `pyproject.toml`, `packages/cli-npm/package.json`, and `registry/gaia.json` must be identical. Align them automatically:
  ```bash
