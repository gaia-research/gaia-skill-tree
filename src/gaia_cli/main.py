...
def init_command(args):
    ...
    if os.path.exists("registry/gaia.json"):
        ...
        if os.path.exists(hook_script):
            print(
                "  git hooks:  found hook script (run manually if trusted: sh scripts/install-git-hooks.sh)"
            )
                ...
                if not canon and ("/" in display or sid in ctx.novel_ids):
                    ...
                    if display.startswith("/"):
                        ...
                        if len(parts) == 2:
                            handle_color = (
                                COLOR_REDACTED
                                if parts[0] == REDACTED_BLOCK
                                else COLOR_CONTRIBUTOR
                            )
        ...
        if installed_skills:
            ...
            for sk in installed_skills:
                ...
                if match:
                    ...
                    rank_color = RANK_COLORS.get(
                        canon_sk.get("level", "0★"), RANK_COLORS["0★"]
                    )
                ...
                print(
                    f"  ○ {user_label} custom skill (found in {location}){match_note}"
                )
...
def demote_command(args):
    """Run demotion flow for a skill in a user's tree."""
    config = load_config()
    if not config:
        print("Gaia not initialized.", file=sys.stderr)
        sys.exit(1)

    username = args.user or config.get('gaiaUser')
    if not username:
        print("No gaiaUser set in config and --user not provided.", file=sys.stderr)
        sys.exit(1)

    skill_id = args.skillId.strip().lstrip("/")

    try:
        from .promotion import demote_skill
        result = demote_skill(username, skill_id, args.registry)

        display_name = result.get('displayName') or skill_id
        if result.get('newLevel') is None:
            print(f"\n✦ {display_name} removed from {username}'s tree (was Level {result['previousLevel']}).\n")
        else:
            level_name = LEVEL_NAMES.get(result["newLevel"], result["newLevel"])
            print(f"\n✦ {display_name} demoted to Level {result['newLevel']} ({level_name}) in {username}'s tree.\n")
    except ValueError as exc:
        print(str(exc), file=sys.stderr)
        sys.exit(1)
...
def push_command(args):
    ...
    if not batch.get("proposedSkills") and not batch.get("knownSkills"):
        print(
            "Error: No skills to be pushed. Please install newer skills then gaia scan, or fuse custom skills before pushing.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if pushable_custom_skills:
        ...
        try:
            user_input = input(
                "Please select which skills you do NOT want to push (space or comma separated numbers, or press Enter to push all): "
            )
        ...
        if excluded_ids:
            batch["proposedSkills"] = [
                s
                for s in batch.get("proposedSkills", [])
                if s["id"] not in excluded_ids
            ]
            batch["knownSkills"] = [
                s
                for s in batch.get("knownSkills", [])
                if s["skillId"] not in excluded_ids
            ]
            batch["similarity"] = [
                s
                for s in batch.get("similarity", [])
                if s.get("sourceSkillId") not in excluded_ids
            ]
...
def get_parser():
    ...
    demote_parser = subparsers.add_parser('demote', help="Demote or remove a skill from a local user's skill tree")
    demote_parser.add_argument('skillId', help="Skill ID to demote")
    demote_parser.add_argument('--user', help="User to demote the skill for (defaults to gaiaUser from config)")
