...
## State Snapshot (2026-08-07 SESSION 2, Program 3 — ladder split into two halves, Program 4 pulled forward, paused on token budget)

Continues the same 2026-08-07 session after a compaction. The FINAL snapshot below is not
superseded — it recorded a true close; work then resumed on founder instruction.

### TLDR
- **The ladder now has two halves and two commands** (founder ruling). Heaven = `off·low·med`,
  subtractive, boot-time only, needs the launcher. Hell = `high·xhigh·max·ultra`, additive,
  works in any live session with only the plugin. **`med` IS `native`** — top of Heaven, not a
  locked rung. **Hell is NOT locked**; `/skill-hell` defaults to `high`.
- The split is a **capability** boundary, not policy: D12 (no eviction mid-session) stopped
  being the blocker and became the reason the line falls where it does.
- **codex-heaven and grok-heaven reach LIVE EXEC.** Both were recipe-only at session start.
  Real spawned authenticated sessions answering (`CODEX_HEAVEN_OK`, `GROK_HEAVEN_OK`).
- **Program 4 (MCP & Summoning, Arc III) pulled forward and substantially DONE** — recorded in
  EPIC 1336. Founder ruling: future work may be done ahead of its arc.
- **Paused on token budget**, not on a blocker. Everything is pushed.

### What changed this session

| Layer | State |
|---|---|
| Ladder model | ✅ Two halves ruled + `docs/LADDER-FLOW.md` written with mermaid flow |
| Ladder implementation | ⏳ 4 commits on `feat/ladder-two-halves`, not yet merged |
| codex-heaven | ✅ live exec — baseline 90 skills/45 enabled → composed 76/0 |
| grok-heaven | ✅ live exec — floor 0, product-floor 9, curated 1, authenticated |
| All 5 doors default | ✅ lowest rung (`off`→product-floor), 0 skills / 0 standing tokens |
| Trust generalization | ✅ optional + tree-provided; no-trust tree ranks by relevance and SAYS so |
| Summon cards / `--count` / warm re-attach | ✅ ceiling 5; re-attach 0.000s, `cacheSource: session` |
| gaia-mcp naming | ✅ merged — `TREE_URL`, `TREE_NAMED_URL`, `TREE_CONTRACT_VERSION`, `SKILL_HELL_*` |
| README ladder diagram | ✅ SVG from site design tokens, generated by `scripts/gen-ladder-svg.py` |
| EPIC 1336 | ✅ Program 3 rewritten; Program 4 recorded as pulled-forward |
| Delivery/install plan | ✅ `skill-heaven/docs/DELIVERY-PLAN.md` (Opus-drafted) |
| Delivery/install execution | ❌ not started |
| GitHub Pages | ❌ enabled but `"status":"errored"` — points at monorepo root |

### Branches at end of session

| Repo | Branch | Head | Status |
|---|---|---|---|
| skill-heaven | `integration/program-3-prototypes` | `325e6bf` | pushed; PR #39 draft, founder-gated |
| skill-heaven | `feat/ladder-two-halves` | `7e0a48a` | pushed; **unmerged**, 4 commits |
| gaia-mcp | `main` | `48ce35b` | naming merged (#9) |
| gaia-mcp | `feat/ambient-summon` | `d93c67a` | pushed, rebased onto squashed main; **PR #10 open** |
| gaia-skill-tree | `founder/program-3-prototypes` | `06a356cb1` | pushed |

### Issues + PRs touched
- gaia-mcp **#9 merged** (tree-agnostic naming), **#10 open** (ambient summon), **#8 open**
  (release-please 0.2.0, not yet merged).
- skill-heaven #39 draft integration PR. #41 filed earlier for cross-platform.
- gaia-skill-tree **EPIC #1336 body rewritten**.

### THE SHIP BLOCKER (verified, not assumed)
npm serves `@gaia-research/mcp@0.1.0`, which **ships only the `gaia-mcp` binary — no
`skill-hell`**. `/skill-hell` therefore works for **nobody** installing from npm until 0.2.0.
Publishing runs under GitHub Environment `npm` with a `required_reviewers` rule
(`mbtiongson1`, `nova-gaia`, `milim-gaia`), OIDC trusted publishing, no token. **The founder
must click approve.** Do not propose removing the gate.

### Lessons / hazards preserved
- **NEVER force-push `main` in skill-heaven.** `main` holds 3 `packages/site` commits the
  integration branch lacks; a force-push deletes `packages/site`. Merge-commit only.
- `packages/site` **is** on `main`, and main's copy is the newer pruned one — deploy Pages from
  `main`, not `feat/site-landing-prototype`. (Corrects an earlier assumption of mine.)
- **npx is impossible today**: every door shim resolves `tsx`, but `tsx` is only a *root
  devDependency*. Verified by packing core and installing clean → `Cannot find module
  'tsx/package.json'`. Git clone is the honest recommendation until ~2 lines/package change.
- `main`'s Claude plugin has **no** `skill-hell.md` / `render-hell.mjs` — integration branch only.
- Squash-merging a PR orphans branches cut from it; rebase `--onto origin/main <old-base>`.
- Claude Code builds its skill listing **at boot**; there is no mid-session load path. A skill
  dir written mid-session is on disk but invisible to the model. The summon **card** is what
  makes it known — which is also far cheaper than pasting the whole body.
- I introduced the original ranking bug via a brief that made rating the primary sort key
  (`garrytan/gstack` won every query). Briefs are load-bearing; workers follow them exactly.
- "registry" is **not** a banned term — V5-8 rejected `gaia.registry` as a *namespace* only.
  `TREE_URL` is still correct, for the different reason that registry names a Gaia surface.

### Open questions for next orchestrator
- **Ambient trigger**: agent-initiated (ships now, only fires when the model recognizes its own
  gap) vs a per-turn hook (truly ambient, noisier, pricier). Founder call. See C10 in
  `founder/PROGRAM-3-CHALLENGES.md`.
- Does a **card alone** let Claude actually invoke a summoned skill, or must the body stay?
  Probe assigned to WP16; pi may differ (it has a real native load path). A genuine
  Claude/pi asymmetry is a product fact, not a bug.
- `ultra` remains **unratified** — refuses as such rather than as gated. Needs a founder ruling.
- Ecosystem surfaces flagging skill-heaven/skill-hell as "in public testing" — **not done**;
  gaia-skill-tree frontend changes are human-gated, so build as a PR and hold.

### Token cost (this session)
Continued past the ~$177 mark recorded at the prior snapshot; paused on founder instruction
when budget ran out. Per-worker at pause: ladder ~$4.9, exec-doors ~$0.83, summon-gc ~$9.0
(pi/codex sub-costs). Exact figure not re-measured at pause — do not quote a precise total.
