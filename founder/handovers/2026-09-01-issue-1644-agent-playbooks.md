...
> **Implementation reconciliation (2026-09-01):** the restoration stack advanced after this plan's first commit. The live base now already wires `cli_contract_drift`, so P2 reduces to correcting its stale routine header. The live grading contract also has typed per-row thresholds plus an aggregate `--trust` fallback, so P1 must document that three-way distinction rather than preserve the simpler two-scale diagnosis below. Finally, implementation review proved D9 stronger than first reported: `gaia-quick-curate` advertises a 25-state auto-merge runtime that the paused `gaia curate` scaffold does not implement. That defect is tracked as subtask [#1679](https://github.com/gaia-research/gaia-skill-tree/issues/1679). Live code remains authoritative where this historical diagnosis differs.
...
| **D2** | The word **grade** spans three distinct calculations without a complete operational explanation. | `META.md:104` still presents one flat row scale, while `registry/schema/meta.json` now defines evidence-type-specific `perRowGradeThresholds`; `gaia dev evidence` grades typed rows from `artifact_score` and falls back to aggregate `--trust` thresholds. Overall Trust Grade is separately derived from total Trust Magnitude. | An agent can choose the wrong input or threshold family while still producing valid-looking output. P1 must name all three paths and their precedence; no threshold changes. |
...
| **D4** | One authorization failure still states the violation, not the remedy. | General operator gating already explains the 4★ path, `gaia whoami`, and CI override. The stricter real-verifier check in `commands/dev/verify.py` remains a dead end and intentionally does not accept the override. | An agent reaching the stricter verification gate cannot tell which legitimate next action resolves it. P4 owns this remaining remediation. |
...
| **D8** | The drift sensor is wired, but its routine contract still claims it is not. | The restored Steward fleet emits and routes `cli_contract_drift`, with registration tests, while `founder/steward/routines/cli-contract-drift.md` still says *"(not wired — no sensor emits this debt yet)"*. | The detector runs, but the human/agent contract lies about operational state. P2 is now a documentation reconciliation, not a new sensor implementation. |
...
| **P2 — reconcile drift-sensor truth** | Correct the stale routine header and verify the already-restored `cli_contract_drift` sensor, policy route, and registration test. No duplicate sensor implementation. | A | P1 |
