...
- **Phase 2B — `ev-benchmark-verification`:** verifies `benchmark-result` rows and optional candidate manifests against `registry/benchmark-sources.json`. It classifies generic-applicable registered benchmark sources, scoring eligibility, citation-only/pending/candidate-only rows, rejected/retired usage, unknown benchmarkIds, missing reproducibility fields, and missing/dubious percentiles without mutating the registry. Registered benchmark sources may declare `appliesToGenericSkillRefs` to say which generic capability buckets they evaluate, but they remain non-scoring until a named `benchmark-result` row satisfies approved provenance and reproducibility requirements.
...
Evidence is additive, not pass/fail. New rows can arrive from Stage 1 minimum rows (`github-stars-own`, `repo-own`, `self-attestation`) or from skippable Phase 0 discovery (`benchmark-result`, `arxiv`, `peer-review`, richer `social-signal`). Benchmark rows add a dedicated human gate: after Phase 2B, Phase 3, and Phase 4, humans must approve benchmark catalog promotion or `/gaia-ingest-batch`; machines classify, humans promote. A registered benchmark source is generic-applicable catalog metadata only: it can name the generic refs the benchmark applies to, but it creates no named-skill score, no Trust Magnitude contribution, and no scoring evidence row until a named `benchmark-result` row satisfies reproducibility and approved provenance. Trust Magnitude and rank are recomputed at appraisal time.
