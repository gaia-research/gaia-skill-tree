...
- **Origin Status**: The **most renowned** implementation in a generic bucket earns "Origin" — the highest-rated named skill (ties broken by most-attributed / Trust Score), **not** necessarily the earliest. An early implementation may be **superseded** when a stronger one earns the rank. Origin is a mark of merit granted to the implementation that earned it, in keeping with the product motif. Exactly one Origin exists per branch per bucket (at 1★–3★, the Standard branch holds at most one Origin; at 4★+, distinct branches such as Suite and Unique sharing the same generic node may each hold their respective branch Origin). *(Updated 2026-06-02: Origin is merit-based; this supersedes the earlier "first contributor / earliest" rule, since an early entry can be outclassed by a better one. Updated 2026-09-06: Origin is branch-scoped per generic bucket, allowing Suite-branch and Unique-branch implementations sharing the same generic node to each hold Origin standing.)*
