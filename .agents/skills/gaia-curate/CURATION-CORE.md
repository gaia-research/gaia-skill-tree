...
This is the canonical read-only contract for the `single`, `checkpointed`, `dynamic`, and `trending` modes of `/gaia-curate`. It is a discovery compiler, not a registry mutation workflow. Modes may schedule and checkpoint packets differently but may not change this lifecycle or cross the L4 stop.
...
Source-native stars, commit counts, contributor counts, and listing signals may be preserved as discovery observations when already present. They are prioritization/provenance facts only: do not convert them into canonical evidence rows, search for additional evidence, score evidence, assign grades or classes, set a star level, or calculate Trust Magnitude. Do not mutate the registry, regenerate docs, create intake, use Git, or open a PR; all modes stop at L4.
