(function () {
    ...
    function draw() {
      ...
      nodes.forEach(({ skill }) => {
        ...
        if (state.colorMode === 'cluster' && skill.cluster !== undefined) {
          ...
          const hex = CLUSTER_COLORS[skill.cluster % CLUSTER_COLORS.length];
  ...
  _graphCloseOverlay.querySelector('[data-graph-random]').addEventListener('click', () => {
