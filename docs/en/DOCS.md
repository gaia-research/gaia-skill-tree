...
| 3 | `cli-reference.html` | CLI Reference | ✅ Done — documented missing mutating dev verbs (`dev rm-evidence`, `dev rm`, `dev link`, `dev reclassify`, `dev update-named`, `dev build`) 039; documented `gaia steward` suite (`scan`, `run`, `dispatch`, `lane`, `verify`, `founder`) 040; documented `gaia dev verify`, `gaia dev diff`, `gaia dev fuse`, `gaia dev arbor` 041; `gaia dev mcp`'s output transcript flagged as literal-not-recommended with a pointer to the `skill-heaven` plugin, matching #1549's reconciliation on `mcp-server.html`/README (editor pass) 047wk; `gaia fuse` card fixed off a Yggdrasil I-era false claim ("inherits the star rank of the highest component skill" — local fuse is levelless per No-self-promote) and the banned verb "Combines"; `dev add` example fixed off an invalid `--type extra` value (Type axis is `basic`/`fusion` only) that also omitted the required `--description` flag (editor pass) 053wk; `gaia fuse` flag table and signature completed with the missing `--skills` and `--delete` flags (both real, confirmed in `impl.py`'s `fuse_parser`), plus custom-fusion and delete examples 053; `gaia dev timeline`'s signature and flag table completed with the `--previous-value`/`--new-value` flags (landed in #1822's frozen-skill-integrity fix, confirmed in `commands/dev/__init__.py` and `timeline.py`), plus a backfill example showing `--new-value` syncing `unlockedSkills` for the Transparency Gate 054 | 002, 017, 018, 025, 034, 039, 040, 041, editor-047wk, editor-053wk, 053, 054 |
...
| 7 | `evidence-classes.html` | Evidence & Trust | ✅ Done — fixed two `--no-build` callouts 035; cleaned up Verifier-tier wording 043; synced `fusion-recipe` row and added a Trust Magnitude / Fusion Score distinction for the Yggdrasil III 0-TM ruling 047; corrected `github-stars-own` to the live logarithmic curve and removed the stale mothership-discount claim 050; added missing `npm-downloads` and `engagement` Evidence Type rows from #1787 052 | 004, 017, 018, 025, 035, 043, 047, 050, 052 |
| 8 | `fusion.html` | Skill Fusion | ✅ Done — Type/Branch model throughout, incl. `gaia fuse`/propose code samples now schema-accurate (`prerequisites`/`combinedFrom`, Type `fusion`) (editor pass closes #1479); "Proposing a New Fusion" batch YAML fixed off the deprecated `class` field to the real `grade`+`type` schema 048; added Fusion Score (Yggdrasil III structural scalar, informational-only) explainer with formula 049 | 004, 017, 018, 025, 029, editor-026wk, 048, 049 |
...
- **Yggdrasil II (ratified 2026-07-07) is current** — see `CONTEXT.md` § Taxonomy v6 before writing tier/rank copy. The legacy Yggdrasil I four-tier taxonomy (Basic ○ / Extra ◇ / Unique ◉ / Ultimate ◆ as *structural* categories) is retired, replaced by a Type axis (`basic`/`fusion`, starless only) + Branch axis (`standard`/`unique`/`suite`, named only, computed from `suiteComponents`). Issue #1479 (migrate `skill-hierarchy.html` and `fusion.html` off the retired four-tier framing) closed 2026-08-15 — both pages now teach the Type/Branch model throughout (see Page Map rows 4, 8). No page still teaches the retired structural framing.
