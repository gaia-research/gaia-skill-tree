...
## Candidate decision matrix — Lane 02B adjudication + maintainer rulings

Lane 02's first-pass proposals have been adjudicated for the 31 escalated rows, followed by explicit maintainer rulings on the ten deferred candidates and upstream suite placement. **These are curation decisions, not evidence verification or approval to implement named skills.** All 95 release-pinned upstream `SKILL.md` files resolved at their raw GitHub URLs (1,120,899 source bytes in total); all have a nonempty description and a frontmatter `name` matching the candidate slug. The four GitHub source repositories are public, not archived, and their current owners match the cited upstream accounts (`addyosmani`, `obra`, `heygen-com`, `garrytan`); the Gaia handle `addy-osmani` differs intentionally from upstream account `addyosmani`. GBrain’s four exceptions (#1893, #1896, #1898, #1902) exist only under the release-pinned `skills/` path, not `plugin/skills/`; the other 67 GBrain source blobs are byte-identical at both locations. Source URLs below point to the inspected file. The named registry and existing generic references were compared with the sources; proposed ranks are conservative 2★ entry estimates, **not** TM-calibrated grades. No upstream claim or popularity number has been treated as verified scoring evidence.

**Gate:** All 31 originally escalated rows have final dispositions; `ESCALATE = 0`. `accepted` remains a candidate for later verified named intake; `mapped` points to an existing named skill and/or existing starless generic reference and creates no duplicate. Maintainer-approved new generic topology is `deprecation-and-migration`, `adversarial-review`, `observability-instrumentation`, and `mcp-server-deployment`; create those generics before their accepted named implementations. **Upstream-suite ruling:** source-repo membership makes the matching existing capstone the default for accepted components (Addy/Agent Skills, Obra/Superpowers, HeyGen/HyperFrames, Garry/GBrain). The sole explicit exception is `garrytan/skill-autobench`, accepted as a standalone skill. `garrytan/skillify` maps to existing `garrytan/skillify` and must not create a second GBrain component. This matrix authorizes neither an automatic suite-manifest mutation nor a fusion-prerequisite change: those implementation diffs remain separately reviewable.
...
| [#1556](https://github.com/gaia-research/gaia-skill-tree/issues/1556) | [`addy-osmani/deprecation-and-migration`](https://github.com/addyosmani/agent-skills/blob/0.6.10/skills/deprecation-and-migration/SKILL.md) | accepted | deprecation-and-migration | 2 | no | Maintainer approved dedicated deprecation/migration generic: sunset and expand/contract compatibility are not framework upgrades or subsystem integration. The approved generic was added first; this 2★ candidate belongs to the upstream Agent Skills suite, subject to normal intake verification. |
...
| [#1558](https://github.com/gaia-research/gaia-skill-tree/issues/1558) | [`addy-osmani/doubt-driven-development`](https://github.com/addyosmani/agent-skills/blob/0.6.10/skills/doubt-driven-development/SKILL.md) | accepted | adversarial-review | 2 | no | Maintainer approved dedicated in-flight fresh-context adversarial-review generic, distinct from self-critique, design questioning, and scripted auto-review. The approved generic was added first; include this candidate in the upstream Agent Skills suite during named intake. |
...
| [#1564](https://github.com/gaia-research/gaia-skill-tree/issues/1564) | [`addy-osmani/observability-and-instrumentation`](https://github.com/addyosmani/agent-skills/blob/0.6.10/skills/observability-and-instrumentation/SKILL.md) | accepted | observability-instrumentation | 2 | no | Maintainer approved new generic after registry check found no general system-observability/instrumentation capability: `token-observability` tracks AI cost and `detect-anomaly` detects deviations, neither emits telemetry. The approved generic was added first; include in the upstream Agent Skills suite during named intake. |
...
| [#1841](https://github.com/gaia-research/gaia-skill-tree/issues/1841) | [`garrytan/book-mirror`](https://github.com/garrytan/gbrain/blob/v0.51.6.0/plugin/skills/book-mirror/SKILL.md) | accepted | document-analyst | 2 | no | Maintainer accepted as a GBrain suite component, not a standalone tool: user-supplied-book chapter analysis with private-context mirroring fits document-analyst; GBrain CLI reliance is appropriate inside this suite. Keep consent, read-only worker, and rights-safe source handling in implementation. |
...
| [#1852](https://github.com/gaia-research/gaia-skill-tree/issues/1852) | [`garrytan/company-brainify`](https://github.com/garrytan/gbrain/blob/v0.51.6.0/plugin/skills/company-brainify/SKILL.md) | omitted | — | — | no | Maintainer rejected independent intake for this batch despite `guardrails` being the closest generic: fail-open exposure of private pages, embedded facts, and history is too risky without independently verified redaction and rollback. Do not add as a GBrain suite component. |
...
| [#1887](https://github.com/gaia-research/gaia-skill-tree/issues/1887) | [`garrytan/research-compendium`](https://github.com/garrytan/gbrain/blob/v0.51.6.0/plugin/skills/research-compendium/SKILL.md) | accepted | research | 2 | no | Maintainer accepted as installable GBrain suite component, changing generic from autonomous-web-research to research: source-by-source synthesis can include books and non-web material. Retain privacy and lawful-retention checks for verbatim archives; repo co-location does not mint evidence. |
...
| [#1892](https://github.com/gaia-research/gaia-skill-tree/issues/1892) | [`garrytan/signal-detector`](https://github.com/garrytan/gbrain/blob/v0.51.6.0/plugin/skills/signal-detector/SKILL.md) | accepted | personal-knowledge-management | 2 | no | Maintainer accepted as a niche GBrain suite component, not a new generic or standalone: opt-in ambient idea/entity capture writes to the user's brain. Upstream documents off-by-default, recorded consent, per-message suppression, and disablement; preserve those boundaries. |
| [#1893](https://github.com/gaia-research/gaia-skill-tree/issues/1893) | [`garrytan/skill-autobench`](https://github.com/garrytan/gbrain/blob/v0.51.6.0/skills/skill-autobench/SKILL.md) | accepted | skill-performance-benchmarking | 2 | no | Maintainer accepted as a STANDALONE exception, not a GBrain suite component: release-tagged root `skills/skill-autobench/SKILL.md` exists upstream. Use that actual blob for installability; retain consent, redaction, and human approval for history-derived evals. |
...
| [#1896](https://github.com/gaia-research/gaia-skill-tree/issues/1896) | [`garrytan/skillify`](https://github.com/garrytan/gbrain/blob/v0.51.6.0/skills/skillify/SKILL.md) | mapped | garrytan/skillify; skill-authoring | — | no | Maintainer removed duplicate GBrain skillify proposal: existing `garrytan/skillify` under the gstack source and skill-authoring generic remains canonical. Do not overwrite its source, create `gbrain-skillify`, or add a duplicate GBrain suite component. |
...
| [#1913](https://github.com/gaia-research/gaia-skill-tree/issues/1913) | [`garrytan/remote-mcp`](https://github.com/garrytan/gbrain/blob/v0.51.6.0/plugin/skills/remote-mcp/SKILL.md) | accepted | mcp-server-deployment | 2 | no | Maintainer approved new generic for deploying/exposing an EXISTING MCP server, distinct from client integration and server creation. The approved generic was added first; this 2★ GBrain suite candidate must preserve explicit Funnel consent, bearer/OAuth checks, scoped grants, and real client verification. |

### Final counts after maintainer rulings
...
**Against Lane 02's original proposals**, 8 of the 31 escalated dispositions changed: #1910, #1846, #1848, #1852, #1871, #1892, #1896, #1904. Lane 02B had provisionally changed 12 before maintainer rulings reversed seven of those; this table and the 8-row count are the final decision record. **No non-escalated row changed.** For the 38 accepted: 12 default to `addy-osmani/agent-skills`, 2 to `obra/superpowers`, 1 to `heygen-com/hyperframes`, 22 to `garrytan/gbrain`, and 1 (`garrytan/skill-autobench`) is standalone. These are placement decisions; no manifest or fusion edge was changed by this matrix update.
