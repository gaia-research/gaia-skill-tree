...
| **schema/** | `registry/schema/`, `src/gaia_cli/data/registry/schema/`, `docs/schema/` | The schema directories must move in lockstep; schema PRs touching only one side always trip CI. Codified in `.github/workflows/branch-scope.yml`; do not require `skip-scope-check`. |
