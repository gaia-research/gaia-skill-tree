"""Unit tests for gaia dev rename (#791, #1456)."""
...
def test_rename_updates_user_tree_references(tmp_path):
    """skillId, combinedFrom[], timeline[].skillId, and timeline[].details
    all update together for a named-skill rename."""
    _named_registry(tmp_path)
    ...
    register_events = [e for e in tree["timeline"] if e["action"] == "register"]
    assert register_events[0]["skillId"] == "acme/new-slug"
    assert register_events[0]["details"] == "Registered named skill acme/new-slug"


def test_rename_logs_user_tree_timeline_event(tmp_path):
    """The sibling schema PR (#1456) legalizes 'rename' as a user-tree
    timeline action; the rename command must actually log one."""
    _named_registry(tmp_path)
    tree_path = _write_tree(tmp_path, "someuser", "acme/old-slug")
    ...
    tree = json.loads(tree_path.read_text(encoding="utf-8"))
    rename_events = [e for e in tree["timeline"] if e["action"] == "rename"]
    assert len(rename_events) == 1
    assert rename_events[0]["skillId"] == "acme/new-slug"
    assert rename_events[0]["details"] == "Renamed from acme/old-slug to acme/new-slug"
...
def test_stale_reference_report_flags_leftover_reference(tmp_path, capsys):
    ...
    and generated artifacts under it must be disclosed as out of scope."""
    _named_registry(tmp_path)
...
def test_generic_rename_updates_user_tree_and_reports(tmp_path, capsys):
    ...
    rename_events = [e for e in tree["timeline"] if e["action"] == "rename"]
    assert rename_events[0]["skillId"] == "skill-new"
