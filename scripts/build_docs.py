...
def main(argv: list[str] | None = None) -> int:
    ...
    if badges_changed and args.check:
        print(
            "::warning::docs/badges/ is stale (warn-only — human-curated infra/badge-* "
            "artifact, not regenerated by the auto-sync runner). "
            "Run `python scripts/generateBadges.py` locally and commit the results.",
            file=sys.stderr,
        )
    ...
    changed = (
        ...
        or profiles_changed
        # badges_changed: intentionally omitted — see warn-only block above (human-curated
        # infra/badge-* artifact, not regenerated by the auto-sync runner).
