---
id: leonxlnx/taste-skill
name: taste-skill
contributor: leonxlnx
origin: false
genericSkillRef: design-generation
status: named
level: 4★
description: Anti-slop frontend master skill for landing pages, portfolios, and redesigns.
  Infers design direction, dials visual variance and density, applies real design
  systems, and runs strict pre-flight checks.
createdAt: '2026-08-22'
updatedAt: '2026-08-30'
title: Taste Skill
links:
  github: https://github.com/leonxlnx/taste-skill/blob/main/skills/taste-skill/SKILL.md
timeline:
- timestamp: '2026-08-22T07:11:01Z'
  action: add
  contributor: mbtiongson1
  details: Added named skill leonxlnx/taste-skill
- timestamp: '2026-08-22T07:11:07Z'
  action: evidence_added
  contributor: mbtiongson1
  details: 'Added evidence from https://github.com/leonxlnx/taste-skill/blob/main/skills/taste-skill/SKILL.md
    (type: github-stars-own)'
- timestamp: '2026-08-22T07:11:11Z'
  action: evidence_added
  contributor: mbtiongson1
  details: 'Added evidence from https://github.com/leonxlnx/taste-skill (type: repo-own)'
- timestamp: '2026-08-22T07:13:42Z'
  action: rank_up
  contributor: mbtiongson1
  details: Calibrated level from 2★ to 4★
- timestamp: '2026-08-22T08:08:52Z'
  action: suite_ref_set
  contributor: mbtiongson1
  details: 'Updated via `gaia dev fuse`: suiteRef=leonxlnx/taste-skill, suiteComponents+=[''leonxlnx/redesign-skill'',
    ''leonxlnx/brandkit'', ''leonxlnx/imagegen-frontend-web'', ''leonxlnx/imagegen-frontend-mobile'',
    ''leonxlnx/gpt-tasteskill'', ''leonxlnx/brutalist-skill'', ''leonxlnx/minimalist-skill'',
    ''leonxlnx/soft-skill'', ''leonxlnx/stitch-skill'', ''leonxlnx/image-to-code-skill'',
    ''leonxlnx/output-skill''].'
- timestamp: '2026-08-29T17:15:51Z'
  action: recalibrate_trust_magnitude
  contributor: mbtiongson1
  details: 'TM (none) -> 286.0, grade (none) -> A (gaia dev calibrate-trust-magnitude;
    Issue #1600)'
evidence:
- source: https://github.com/leonxlnx/taste-skill/blob/main/skills/taste-skill/SKILL.md
  evaluator: mbtiongson1
  date: '2026-08-22'
  type: github-stars-own
  notes: 79.0k stars on Leonxlnx/taste-skill repository
  stars: 79067
  sourceStartedAt: '2026-02-19'
  grade: A
- source: https://github.com/leonxlnx/taste-skill
  evaluator: mbtiongson1
  date: '2026-08-22'
  type: repo-own
  notes: 152 commits, 6 contributors, MIT license
  commits: 152
  contributors: 6
  sourceStartedAt: '2026-02-19'
  grade: B
verification:
  firstEvidenceAt: '2026-08-22T07:11:07Z'
suiteComponents:
- leonxlnx/brandkit
- leonxlnx/brutalist-skill
- leonxlnx/gpt-tasteskill
- leonxlnx/image-to-code-skill
- leonxlnx/imagegen-frontend-mobile
- leonxlnx/imagegen-frontend-web
- leonxlnx/minimalist-skill
- leonxlnx/output-skill
- leonxlnx/redesign-skill
- leonxlnx/soft-skill
- leonxlnx/stitch-skill
trustMagnitude: 286.0
overallTrustGrade: A
trustMagnitudeInputHash: 8eaaa40d3b7c3b11a686c0810ee71ca4a583edee301e69eb461af1ccc6961f4a
---

## Installation
Add installation instructions here.
